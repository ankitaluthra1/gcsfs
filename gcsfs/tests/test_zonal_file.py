...
def test_zonal_file_prefetcher_initialization(mock_gcsfs):
    ...
    with (
        ...
        assert isinstance(zf.cache, Prefetcher)
...
async def test_fetch_logical_chunk_split_logic(mock_gcsfs):
    ...
    with (
        ...
        # Create a mock that actually behaves like an async context manager
        ...
        # Remove the asyncio.gather mock so the event loop actually runs our tasks
        ...
        # Patch the missing close() for ReadAheadChunked just for the test
        if not hasattr(zf.cache, "close"):
            zf.cache.close = mock.Mock()
