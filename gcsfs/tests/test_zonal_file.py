...
def mock_gcsfs():
    ...
    fs.loop = mock.Mock()
    # New: stub the MRDPoolCache so ZonalFile.__init__ doesn't try real RPCs.
    mock_pool = mock.Mock()
    mock_pool.persisted_size = 1000
    fs._shared_mrd_pool = mock.Mock()
    fs._shared_mrd_pool.get = mock.AsyncMock(return_value=mock_pool)
    return fs
...
async def test_zonal_file_open_shares_idle_queue(init_mrd_mock):
    ...
    fs._shared_mrd_pool = MRDPoolCache(fs, max_idle_pools=8)

    # Drive two opens through the real cache; bypass the rest of ZonalFile/GCSFile init.
    pool_a = await fs._shared_mrd_pool.get("bucket", "key", "1", pool_size=1)
    pool_b = await fs._shared_mrd_pool.get("bucket", "key", "1", pool_size=1)

    assert fs._shared_mrd_pool._refcounts[("bucket", "key", "1")] == 2
    ...
    pool_c = await fs._shared_mrd_pool.get("bucket", "key", "1", pool_size=1)
    mrd_c = pool_c._all_mrds[0]
    await pool_c.close()

    pool_d = await fs._shared_mrd_pool.get("bucket", "key", "1", pool_size=1)
    assert pool_d._all_mrds[0] is mrd_c

    assert init_mrd_mock.await_count == 2  # still only called twice

    await pool_d.close()
    await fs._shared_mrd_pool.close()
    fs.loop.close()
