...
def test_prefetcher_partial_read_and_remainder(
    ...
    # 6 bytes should be left in the remainder buffer
    assert len(cache._remainder) == 6
    assert cache._buffer_offset == 4

    # Fetch next 8 bytes (4 to 12).
    # Should consume the 6 byte remainder and pull 2 bytes from the next block.
    res2 = cache._fetch(4, 12)
    assert res2 == prefetcher_source_data[4:12]

    # Remainder should now have 8 bytes left from the second block
    assert len(cache._remainder) == 8
    assert cache._buffer_offset == 12
...
def test_prefetcher_cross_block_read(prefetcher_setup, prefetcher_source_data):
    ...
    assert len(cache._remainder) == 5


def test_prefetcher_seek_same_offset(prefetcher_setup, prefetcher_source_data):
