...
class Prefetcher(BaseCache):
    ...
    the prefetch loop. It strictly tracks hit, miss, and byte request
    statistics as defined in `BaseCache`.
    ...
    fetcher : Callable
        Function or coroutine of the form `f(start, end)` which gets bytes from the remote.
    size : int
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_prefetch_size = max(
            max_prefetch_size or 0, 2 * self.blocksize, 128 * 1024 * 1024
        )
